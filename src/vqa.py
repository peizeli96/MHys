...
from utils import load_state_dict, LossMeter, set_global_logging_level, Logger
import dist_utils
# import wandb
import os
...
class Trainer(TrainerBase):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            # Test Set
            last_path = os.path.join(self.args.output, "LAST")
            self.load(last_path, loc="cuda:1")
            # import pdb; pdb.set_trace()
            quesid2ans = self.predict(self.test_loader,'predict_%d.json' % epoch)



            # if self.args.submit:
            #     dump_path = os.path.join(self.args.output, 'predict_%d.json' % epoch )
            #     self.predict(self.submit_test_loader, dump_path)

            #     dump_path_add = os.path.join(self.args.output, 'predict_%d_add.json' % epoch )
            #     self.predict(self.submit_test_loader, dump_path_add)
    ...
    def predict(self, loader, dump_path=None):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(loader):
                ...
                # pred_ans_add = results['pred_ans_add']
                # pred_ans_sum = results['pred_ans_sum']
                ...
                # for qid, ans in zip(ques_ids, pred_ans_add):
                #     quesid2ans_add[qid] = ans   

                # for qid, ans in zip(ques_ids, pred_ans_sum):
                #     quesid2ans_sum[qid] = ans              
        ...
        # qid2ans_list_add = dist_utils.all_gather(quesid2ans_add)
        # if self.verbose:
        #     quesid2ans_add = {}
        #     for qid2ans in qid2ans_list_add:
        #         for k, v in qid2ans.items():
        #             quesid2ans_add[k] = v

        #     if dump_path_add is not None:
        #         evaluator = loader.evaluator
        #         evaluator.dump_result(quesid2ans_add, dump_path_add)

        # qid2ans_list_sum = dist_utils.all_gather(quesid2ans_sum)
        # if self.verbose:
        #     quesid2ans_sum = {}
        #     for qid2ans in qid2ans_list_sum:
        #         for k, v in qid2ans.items():
        #             quesid2ans_sum[k] = v

        #     if dump_path_sum is not None:
        #         evaluator = loader.evaluator
        #         evaluator.dump_result(quesid2ans_sum, dump_path_sum)
